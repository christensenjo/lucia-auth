...
async function handleSubmit() {
    try {
        ...
        const { data: token } = await useFetch('/api/session/generateSessionToken', {
            method: 'POST'
        });

        // Create session with token
        const { data: sessionResponse } = await useFetch('/api/session/createSession', {
            method: 'POST',
            body: {
                token,
                email: email.value,
        ...
        // Only navigate if we got back a valid session
